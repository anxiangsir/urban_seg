...
with tf.Session(config=gpu_config) as sess:
    ...
    for step in range(1, 100000):
        ...
        #
        loss_tr, acc_tr, _, summary_train = sess.run([model.loss, model.pixel_acc, train_op, model.summary_op],
                                                     feed_dict={model.x: x_tr, model.y: y_tr})
        loss_val, acc_val, summary_val = sess.run([model.loss, model.pixel_acc, model.summary_op],
        ...
        if (step % display == 0):
            ...
            logging.info(
                "Iter {:}, loss_tr= {:.4f}, acc_tr= {:.4f}, loss_val= {:.4f}, acc_val= {:.4f} ".
                    format(step, total_loss_tr / display, total_acc_tr / display, total_loss_val / display,
                           total_acc_val / display))
